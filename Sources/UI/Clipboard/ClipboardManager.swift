...
class ClipboardManager: ObservableObject {
    ...
    func loadSettings() {
        if let data = UserDefaults.standard.data(forKey: rulesKey),
           let decoded = try? JSONDecoder().decode([ClipboardPreferenceRule].self, from: data) {
            ...
            self.customRules = [
                ClipboardPreferenceRule(appName: String(localized: "clipboard-manager.default-rule.keychain-access", defaultValue: "Keychain Access", bundle: .main, comment: "Default clipboard rule app name for Keychain Access"), ruleType: .temporary),
    ...
    private func checkPasteboard() {
        ...
        if let newText = pasteboard.string(forType: .string), !newText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            let appName = activeApp?.localizedName ?? String(localized: "clipboard-manager.source-app.unknown", defaultValue: "Unknown", bundle: .main, comment: "Fallback source app name when clipboard source is unavailable")
