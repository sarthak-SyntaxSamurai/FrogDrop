        ...
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                if let timer = activeTimer {
                    ...
                    Text(timer.name.isEmpty ? String(localized: "timer.floating-pill.state.focusing", defaultValue: "Focusing", comment: "Status text shown in floating focus pill when active timer has no custom name") : timer.name)
                        .font(.system(size: 9, weight: .medium, design: .rounded))
                        .foregroundColor(.white.opacity(0.7))
                        .lineLimit(1)
                } else {
                    Text(String(localized: "timer.floating-pill.state.ready-to-focus", defaultValue: "Ready to Focus", comment: "Status text shown in floating focus pill when no timer is active"))
