...
class PDFToolkit {
    ...
    func mergePDFs(urls: [URL]) async -> URL? {
        ...
        if mergedDocument.write(to: destURL) {
            ...
            notifyUser(
                title: String(
                    localized: "pdf-toolkit.merge-complete.title",
                    defaultValue: "PDFs Merged",
                    bundle: .main,
                    comment: "Notification subtitle when PDF merge completes"
                ),
                body: String(format: String(
                    localized: "pdf-toolkit.merge-complete.body",
                    defaultValue: "Combined %d pages into %@",
                    bundle: .main,
                    comment: "Notification body showing merged page count and output filename"
                ), pageIndex, destURL.lastPathComponent)
            )
