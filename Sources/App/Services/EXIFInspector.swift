    ...
    static func analyze(url: URL) -> EXIFReport {
        ...
        if let expTime = exifDict?[kCGImagePropertyExifExposureTime] as? Double {
            if expTime < 1.0 && expTime > 0 {
                shutter = "1/\(Int(round(1.0 / expTime)))s"
            } else {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                VStack(spacing: 12) {
                    ...
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(report.isClean ? "100% Clean Image" : "Sensitive Metadata Detected")
                                .font(.system(size: 12, weight: .bold, design: .rounded))
                                .foregroundColor(report.isClean ? .green : .orange)
                            
                            Text(report.isClean ? "No GPS location, camera serial, or sensitive tags found." : "Contains location or camera information that can identify where & when it was taken.")
                    ...
                    if report.cameraModel != nil || report.cameraMake != nil || report.lensModel != nil {
                        VStack(alignment: .leading, spacing: 6) {
                            Label("Camera & Optics", systemImage: "camera.fill")
                                .font(.system(size: 10, weight: .bold))
                                .foregroundColor(.blue)
                            
                            if let make = report.cameraMake, let model = report.cameraModel {
                                MetaRow(label: "Device", value: "\(make) \(model)")
                            } else if let model = report.cameraModel {
                                MetaRow(label: "Device", value: model)
                            }
                    ...
                    // File Details Card
                    VStack(alignment: .leading, spacing: 6) {
                        Label("File & Resolution", systemImage: "doc.fill")
                            .font(.system(size: 10, weight: .bold))
                            .foregroundColor(.secondary)
                        
                        if let w = report.pixelWidth, let h = report.pixelHeight {
                            MetaRow(label: "Dimensions", value: "\(w) × \(h) px")
                        }
                        MetaRow(label: "File Size", value: report.fileSizeString)
                        if let date = report.dateTimeOriginal {
                            MetaRow(label: "Captured", value: date)
                        }
                        if let profile = report.colorProfile {
                            MetaRow(label: "Color Profile", value: profile)
                        }
                    }
                    .padding(10)
                    .frame(maxWidth: .infinity, alignment: .leading)
            ...
            HStack(spacing: 10) {
                if !report.isClean {
                    Button(action: {
                        ...
                        HStack(spacing: 6) {
                            ...
                            Text(stripSuccess ? "Clean Copy Saved in Downloads!" : "Strip Metadata & Clean")
                        }
                ...
                Button(action: onClose) {
                    Text(report.isClean ? "Done" : "Close")
